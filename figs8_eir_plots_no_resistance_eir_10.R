...
p <- ggplot(eir_data_long) + 
  geom_boxplot(aes(eir/10, run, col = cat, fill = fill)) + 
  facet_wrap(~scenario, ncol = 1, labeller = as_labeller(scenario_names)) + 
  scale_x_continuous(minor_breaks = seq(0, 1, 1/10), breaks = seq(0, 1, 2/10), labels = scales::percent) +
...
print(p)
ggsave("figures/fig_s8.pdf", p, height = 6, width = 8)
ggsave("figures/fig_s8.png", p, height = 6, width = 8)
