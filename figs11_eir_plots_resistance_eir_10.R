...
load("outputs/model_data_runs_new_10.RData")
...
p <- ggplot(eir_data_long %>% 
  ...
  geom_boxplot(aes(eir/10, run, col = resistance, fill = fill)) + 
  facet_grid(scenario~cat, labeller = labeller(scenario = scenario_names)) + 
  scale_x_continuous(minor_breaks = seq(0 , 10, 1/1), breaks = seq(0, 1, 2/10), labels = scales::percent) +
...
print(p)
ggsave("figures/fig_s11.pdf", p, height = 7, width = 12)
ggsave("figures/fig_s11.png", p, height = 7, width = 12)
